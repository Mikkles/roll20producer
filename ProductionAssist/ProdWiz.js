const Roll20Pro = (() => {
    ...
    const scriptName = "Roll20 Production Wizard",
        ...
        styles = {
            ...
            note: 'background-color:powderblue; color:gray; ',
            sot: {
        ...
        handleInput = function (msg) {
            ...
            if (msg.content.search(/^(!ta|!sortta)\b/) !== -1) {
                ...
                if (args.includes("help")) {
                    ...
                    let footer = '</div>';
                    sendChat("Token Action Maker", `/w ${whom} ${header}${helpText}${footer}`);
                    return;
                }
            }
            ...
            if (msg.content.search(/^!ta\b/) !== -1) {
                ...
                if (args.includes("help")) {
                    ...
                    let footer = '</div>';
                    sendChat("Token Action Maker", "/w " + msg.who + header + helpText + footer);
                    return;
                }
                ...
                if (sheet === "5e") {
                    _.each(char, function (a) {
                        ...
                        sendChat("Token Action Maker", `/w ${whom} Created 5e Token Actions for ${a.get('name')}.`);
                    });
                    ...
                    _.each(char, function (a) {//PF2 NPC
                        ...
                        sendChat("Token Action Maker", `/w ${whom} Created PF2 Token Actions for ${a.get('name')}.`);
                    });
                }
                ...
                _.each(char, function (d) {
                    ...
                    sendChat("Token Action Maker", `/w ${whom} Deleted all unprotected Token Actions for ${d.get('name')}.`);
                });
                ...
                _.each(char, function (d) {
                    ...
                    sendChat("Token Action Maker", `/w ${whom} Deleted all Token Actions for ${d.get('name')}.`);
                });
                ...
                if (sheet === "5e") {
                    ...
                    _.each(char, function (a) {
                        if ((parseInt(isNpc(a.id)) )=== 1) {//5e NPC Sorted
                            ...
                            if (args.includes("attacks")) {
                                sortRepeating(/repeating_attack_[^_]+_atkname\b/, 'repeating_attack_%%RID%%_attack', a.id, usename);
                            }
                            if (args.includes("traits")) {
                                createRepeating(/repeating_traits_[^_]+_name\b/, 'repeating_traits_%%RID%%_output', a.id, usename);
                            }
                    ...
                    _.each(char, function (a) {//not recommended for PF2. A warning will be sent and unsorted actions will be created.
                        ...
                        sendChat("Token Action Maker", `/w ${whom} **Using !sortta for Pathfinder characters is not recommended. Alphabetization destroys the logical order of the *Attack-Attack2-Attack3* progression.**<BR><BR>Created non-sorted PF2 Token Actions for ${a.get('name')}.`);
